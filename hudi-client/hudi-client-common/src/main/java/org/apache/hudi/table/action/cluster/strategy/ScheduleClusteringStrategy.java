...
public abstract class ScheduleClusteringStrategy<T extends HoodieRecordPayload,I,K,O> implements Serializable {
  ...
  protected List<FileSlice> getFileSlicesEligibleForClustering(String partition) {
    ...
    return hoodieTable.getSliceView().getLatestFileSlices(partition)
        ...
        .filter(slice -> slice.getBaseFile().map(HoodieBaseFile::getFileSize).orElse(0L) < writeConfig.getClusteringTargetFileMaxBytes())
  ...
  protected Map<String, Double> buildMetrics(List<FileSlice> fileSlices) {
    ...
    FileSliceUtils.addFileSliceCommonMetrics(fileSlices, metrics, getWriteConfig().getParquetMaxFileSize());
    return metrics;
  }
