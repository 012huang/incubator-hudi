 ...
 */
public class SparkBulkInsertBasedRunClusteringStrategy<T extends HoodieRecordPayload<T>>
    extends RunClusteringStrategy<T, JavaRDD<HoodieRecord<T>>, JavaRDD<HoodieKey>, JavaRDD<WriteStatus>> {
  private static final Logger LOG = LogManager.getLogger(SparkBulkInsertBasedRunClusteringStrategy.class);

  public SparkBulkInsertBasedRunClusteringStrategy(HoodieSparkCopyOnWriteTable<T> table,
                                                   HoodieSparkEngineContext engineContext,
                                                   HoodieWriteConfig writeConfig) {
    super(table, engineContext, writeConfig);
  }

  public SparkBulkInsertBasedRunClusteringStrategy(HoodieSparkMergeOnReadTable<T> table,
                                                   HoodieSparkEngineContext engineContext,
                                                   HoodieWriteConfig writeConfig) {
    super(table, engineContext, writeConfig);
  }
  ...
  protected Option<BulkInsertPartitioner<T>> getPartitioner(Map<String, String> strategyParams, Schema schema) {
    if (strategyParams.containsKey(SORT_COLUMNS_PROPERTY)) {
      return Option.of(new RDDCustomColumnsSortPartitioner(strategyParams.get(SORT_COLUMNS_PROPERTY).split(","), schema));
    } else {
      return Option.empty();
    }
  }
