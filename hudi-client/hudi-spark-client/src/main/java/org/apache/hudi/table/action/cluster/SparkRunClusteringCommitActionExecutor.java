...
public class SparkRunClusteringCommitActionExecutor<T extends HoodieRecordPayload<T>>
  ...
  /**
   * Submit job to run clustering for the group.
   */
  private CompletableFuture<JavaRDD<WriteStatus>> runClusteringForGroupAsync(HoodieClusteringGroup clusteringGroup, Map<String, String> strategyParams) {
    CompletableFuture<JavaRDD<WriteStatus>> writeStatusesFuture = CompletableFuture.supplyAsync(() -> {
      ...
      return ((RunClusteringStrategy<T, JavaRDD<HoodieRecord<? extends HoodieRecordPayload>>, JavaRDD<HoodieKey>, JavaRDD<WriteStatus>>)
          ReflectionUtils.loadClass(config.getRunClusteringStrategyClass(), table, context, config))
          .performClustering(inputRecords, clusteringGroup.getNumOutputGroups(), instantTime, strategyParams, readerSchema);
    });
